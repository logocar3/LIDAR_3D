...
void SD_file_download(String strText){
    ...
    File download = SD.open("/" + strText); // Zakaj imas argument strText ce ga ne uporabis?
    ...
    if (download) {
      ...
      download.close();  // Preden zapremo datoteko pustiti procesorju da dokonca zadeve
    }
    else if(!download) { // Ni potrebno se en if, ker je lahko le true/false
...
if(tekst){
      ...
      while(enkoderD<4000){
         ...
         //napaka= enkoderL-enkoderD ;//+ 0.2829;

         napaka= referenca-(enkoderL-enkoderD);//+ 0.2829; 
         ...
         //printf("napaka: %.2f \n",napaka);
         //dutyCycle = dutyCycle + napaka/Kp + napaka_integral*Ki + kd*odvod;

         //up = napaka/Kp + napaka_integral*Ki + kd*odvod;

        // dutyCycleD=dutyCycleD-up/2;
         //dutyCycleL=dutyCycleL+up/2;

         up = up + napaka*Kp + napaka_integral*Ki + kd*odvod;
         ...
         ledcWrite(pwmChannelL, dutyCycleL);
                     ...
                     //zacetek testa
                     //printf("dutyL %.2f , dutyD %.2f  \n", dutyCycleL, dutyCycleD);
